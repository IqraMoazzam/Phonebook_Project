    ...
    if (isset($_POST['submit'])) {
        ...
        if (!$conn) {
            ...
            if (empty($cname) && empty($cnumber)) {
                // echo $active;
                // die;
                $sql = "UPDATE contact_details SET contact_active = {$active}
    WHERE contact_id = (SELECT contact_id FROM contact_details WHERE user_id = {$userid} AND contact_name = '{$oldname}')";
            // echo $sql;
            // die;
            if (mysqli_query($conn, $sql)) {
                ...
                if ($cname != $oldname) {
                    $sql = "UPDATE contact_details SET contact_name = '{$cname}',contact_active = {$active}
                WHERE contact_id = (SELECT contact_id FROM contact_details WHERE user_id = {$userid} AND contact_name = '{$oldname}')";
                ...
                WHERE contact_id = (SELECT contact_id FROM contact_details WHERE user_id = {$userid} AND contact_name = '{$cname}') AND contact_number = '{$oldnumber}'";
        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="breakpoints col-lg-10 col-md-9 col-sm-8 h-auto mt-5">
                    ...
                    <div class="modal mt-3 h-auto" tabindex="-1" id="modal_id">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <!-- <input type="hidden" name="usr_id" value="<?php //echo $_POST['u_id'];?>"> -->
                                    <div class="modal-body">
                                        ...
                                        <div class="mt-3">
                                            ...
                                            <select name="active" class="form-control-sm">
                                                <option value="1">Active</option>
                                                <option value="0">In-Active</option>
                                            </select>
                                        </div>
                                        <input type="hidden" name="u_id" value="<?php echo $_POST['uid']; 
                                                                                        ?>"> 
